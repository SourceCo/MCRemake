      ...
      for (int x = 0; x < toGenerate.size(); x++) {
        try {
          if (Minecraft.chunks.containsKey(toGenerate.get(x))) {
            Minecraft.chunks.get(toGenerate.get(x)).generateTerrain();
            Minecraft.chunks.get(toGenerate.get(x)).left.generateTerrain();
            Minecraft.chunks.get(toGenerate.get(x)).right.generateTerrain();
            Minecraft.chunks.get(toGenerate.get(x)).front.generateTerrain();
            Minecraft.chunks.get(toGenerate.get(x)).back.generateTerrain();
            Minecraft.thr.sendChunk(toGenerate.get(x));
          }
        } 
        catch (Exception e) {
          toGenerate.remove(x);
